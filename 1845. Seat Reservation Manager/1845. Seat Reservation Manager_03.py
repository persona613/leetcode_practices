...
class SeatManager:
    ...
    def heapify(self, i, ln) -> None:
        ...
        l = i * 2 + 1
        r = i * 2 + 2
        if l < ln and self.fs[l] < self.fs[i]:
            self.fs[l], self.fs[i] = self.fs[i], self.fs[l]
            mi = l
        if r < ln and self.fs[r] < self.fs[i]:
            self.fs[r], self.fs[i] = self.fs[i], self.fs[r]
            mi = r
        if mi != i:
            self.heapify(mi, ln)
